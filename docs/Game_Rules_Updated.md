...
- **Rules Note (2026-07-21) — Simultaneous events resolve sequentially.** When multiple events would occur at the same time, they resolve sequentially, one at a time. The player who owns the affected permanents/effects chooses the order; if the events belong to different players, the active player chooses. Listeners and game state are evaluated as of each individual event — a permanent removed by an earlier event in the sequence is not on the board for later events. (This rules a flagged open engine reading — same-ready simultaneous sacrifices previously heard each other's events; it is a ruling on that open question, not a correction of any printed rule.)
