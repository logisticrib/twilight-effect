...
**Rules Note (ruled 2026-08-17) — Item Transfer window when controller and owner differ.** The 2026-07-08 note above ("the controlling player may exhaust a ready character in their party...") was written when controller and owner were always the same player. Control-theft effects (Command the Broken) make them diverge: when a character dies (or otherwise exits) while under a controller who is not its owner, the Item Transfer window belongs to the OWNER, not the controller at time of death. The opposing (controlling/thief) player has no claim on items carried off a companion they were only borrowing. This keeps rescue and burial on the same side — the item transfer window and the Dead Zone the corpse falls into both belong to the owner. (Ratifies the shipped Arc I behavior, commit `524afe6` 2026-08-11 — the owner-routing was implemented and flagged there; confirmed correct, no engine change.)
...
- When a companion flees (its Level exceeds your Willpower at the start of your turn) or a Construct's last Anchor counter is removed at the start of your turn, that card is placed in its owner's Dead Zone along with any items attached to it — not merely "as if" destroyed: both exits ARE sacrifices, and a sacrifice is a death (fleeing re-ruled 2026-07-20; decay ruled 2026-07-15). Fled and decayed cards are therefore recoverable by Dead Zone recursion effects (e.g., Memory Stone, Library of Memory) like any other card there
...
- **Rules Note (owner-ruled 2026-07-24) — The Player Character can never be chosen as a sacrifice.** No effect that has a player sacrifice a permanent — an ability cost, a forced choice (Coercion, "each player sacrifices a permanent or discards a card"), or any other sacrifice — ever offers the Player Character as a legal choice. Rationale: PC death is the loss condition; a "choice" to sacrifice the PC is a choice to lose the game — a trap option, not agency. This promotes the Coercion-specific ruling (2026-07-04) to the general rule; the Coercion entries cross-reference it.
...
- **Rules Note (2026-07-20) — Fleeing is a sacrifice.** A companion that flees is sacrificed: fleeing is a death, fires death-triggered abilities, and interacts with anything that responds to sacrifices, subject to each effect's own scope. Everything in the Dead Zone died to get there. (Supersedes the earlier treatment of fleeing as a non-death exit.)
...
- **Rules Note (ruled 2026-08-21) — FORCED ATTACKS ARE ATTACKS.** An attack a card COMPELS ("each companion you control in the front line attacks target opposing character") is an attack in every respect: it opens the FULL declaration window described above, and every listener that watches for an attack fires on it. Iron Spikes bites a compelled attacker; The Final Word's demand arms against the compelled player, so being coerced into attacking a Final Word board costs BOTH the attack and the sacrifice; a self-hosted retaliation ("whenever this character is attacked") answers a compelled attacker. That severity is honestly derived from the ruling, not an accident of implementation. The R4 and Glass-Cannon corners are unchanged: a compelled 1-HP attacker that dies inside its own declaration window still leaves everything already armed to resolve, and its damage simply never lands. Each attack in a multi-attacker volley declares SEPARATELY and in board order, so an earlier attacker that removes a buff source lowers a later attacker's damage. Engine: fixed once at the `forceAttack` op — it queues one entry per attacker, expanded through the same declaration assembly a chosen attack uses; no listener special-cases compulsion.
...
- **Rules Note (2026-07-13) — Identical simultaneous triggers.** The active player orders simultaneous triggers even when they are identical copies whose order cannot matter. There is no auto-ordering exception. *(SUPERSEDED 2026-07-22 on the chooser: the orderer is now the triggers' owner, not the active player — see the 2026-07-22 Rules Note. The substance stands: identical copies are still actively ordered, with no auto-ordering exception.)*
...
- **Rules Note (2026-07-21) — Simultaneous events resolve sequentially.** When multiple events would occur at the same time, they resolve sequentially, one at a time. The player who owns the affected permanents/effects chooses the order; if the events belong to different players, the active player chooses. *(That cross-ownership clause SUPERSEDED 2026-07-22: when simultaneous events belong to different players, the order is structural — the active player's queue first, the non-active player's above them — not an active-player choice; see the 2026-07-22 Rules Note. The rest of this note stands.)* Listeners and game state are evaluated as of each individual event — a permanent removed by an earlier event in the sequence is not on the board for later events. (This rules a flagged open engine reading — same-ready simultaneous sacrifices previously heard each other's events; it is a ruling on that open question, not a correction of any printed rule.)
- **Rules Note (2026-07-22) — Unified simultaneous ordering.** Each player orders their own simultaneous triggers and events. When both players have simultaneous triggers, the active player's queue onto the stack first and the non-active player's queue above them — so the non-active player's resolve first (LIFO). The same rule governs simultaneous state events (e.g. decay). Listeners and game state are evaluated as of each individual event (unchanged from 2026-07-21). *(Supersedes: the active-player ordering sentence of the 2026-07-12 Trigger Stack note; the 2026-07-13 identical-copies note's chooser; and the cross-ownership clause of the 2026-07-21 sequential-events note — each amended above with a dated pointer.)*
...
**1. Ready Phase**
   ...
   - **Rules Note (owner-ruled 2026-07-22) — Poison counters carry no provenance.** A Poison counter is a Poison counter: the Poison check reads the NUMBER of Poison counters on a character, never how they arrived. Counters applied by a card effect ("put a Poison counter on it") behave identically to counters applied through the POISON keyword's combat-damage trigger — the same die check, the same all-or-none removal, the same 1 damage per counter on a failed check. Rationale: physical play cannot track counter provenance.
