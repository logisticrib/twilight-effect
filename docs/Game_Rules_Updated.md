...
- Characters in the Front Line bear combat (see Targeting Rules); characters in the Back Line are protected from being targeted unless the Front Line is empty, the attacker has Evasive, or the defender has Ranged
...
Existing canon (Card_Design_Parameters §13, quoted verbatim): "**Use a stack** - multiple triggers resolve in order (most recent first)"; (§21, quoted verbatim): "**Resolve most recent first** (last in, first out)", "Your trigger can cause opponent's trigger, which resolves first", ""May" choices made when trigger resolves (no holding)". The Rules Notes below are the owner-ratified operational model (2026-07-12).

- **Rules Note (ruled 2026-07-12) — playing a card uses the stack:** Playing a card puts it on the stack; it does not enter the encounter until the stack empties down to it. "Plays" and "enters" are distinct sequential events: a trigger on "plays a Companion" queues above the companion itself and resolves BEFORE the companion enters.
- **Rules Note (ruled 2026-07-12) — queued triggers survive death:** Once queued, a trigger resolves even if its source or its subject has since died. Ruled example: Tripwire Snare's damage kills the entering 1-HP companion — the companion's already-queued on-enter trigger still resolves. The ruled sequence (verbatim, for Tripwire Snare): companion's enter trigger queues → Tripwire triggers → Tripwire resolves → the enter ability resolves.
- **Rules Note (ruled 2026-07-12) — attack step separation:** Attack declaration and damage are separate steps. Damage does not go on the stack at declaration. Declaration-window triggers ("when/whenever X attacks") resolve during the attack step, before damage is ever queued. If the attacker is dead when the attack step would proceed to damage, damage is never queued — the attack fizzles. (Ruled example: a 1-HP attacker vs Iron Spikes dies to the Spikes damage and deals nothing.)
- **Rules Note (ruled 2026-07-12) — simultaneous triggers:** The ACTIVE player decides the order simultaneous triggers go on the stack. This is pre-existing canon (Rules_Taxonomy Tier 5 #9, Tier 3 #18) and it STANDS — the owner reconfirmed it on 2026-07-12 over an in-session suggestion that the trap controller chooses. Consequence: two defender-owned Tripwire Snares firing on the active player's companion entering are ordered by the active (entering) player.
- **Rules Note (ruled 2026-07-12) — mandatory triggers:** Mandatory triggers fire regardless of whether their effects do anything. Pit Trap has no "may": an already-exhausted mover still trips it — the exhaust is a no-op, the trap still sacrifices itself. (The universal pre-cost refusal rule of 2026-07-08 applies to activated abilities, not mandatory triggers.)
- **Rules Note (ruled 2026-07-12) — trap trigger windows:** Pit Trap triggers on movement only — a companion entering the encounter directly onto the front line does NOT trip it. Iron Spikes triggers at attack declaration (per the trigger-window taxonomy, "attacks" = declaration); its damage resolves during the attack step, before attack damage. Trap self-sacrifice is a sacrifice: it IS a death, fires death triggers, and routes through the shared destruction path (locked ruling 2026-07-08).
