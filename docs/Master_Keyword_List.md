...
- **Rules Note (re-ruled 2026-07-12):** Paranoia triggers on the PLAY: under the trigger stack ("Playing a card puts it on the stack; it does not enter the encounter until the stack empties down to it" — Rules Note 2026-07-12, Game Rules §Core Mechanics), the peek resolves BEFORE the companion enters and before its on-enter effects. Owner ruling 2026-07-12: "Peek first 100%." This SUPERSEDES the 2026-07-04 batch-2 order ruling ("placer's own on-enter scry first, Paranoia peek after"), which was pinned only in tests and is retired with a dated note there — it was never written into this document, so nothing here is deleted. Everything else about Paranoia is unchanged: the controller peeks the placer's deck, top/bottom only, from-hand plays only, and the placing player is held until the peek resolves.
